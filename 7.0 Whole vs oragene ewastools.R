...
w <- estimateLC(beta_whole, ref = "lauren_saliva_ref")
o <- estimateLC(beta_oragene, ref = "lauren_saliva_ref")
...
colnames(w_o_est) #"Epi"       "IC"        "sample_id"  "id"


#saliva percent conversion
w_o_est$Epi <- w_o_est$Epi*100
w_o_est$IC <- w_o_est$IC*100
...
whole_est_long <- gather(whole_estimates, est_celltype, estimates,
...
est <- as.data.frame(estimateLC(meth = beta_samps, ref = "lauren_encode_reinius_ref"))
...
est$sample_id <- pd_samps$sample_id
...
ttest <- t.test(whole_estimates$Epithelial, est$Epi,
                paired = TRUE, alternative = "two.sided")
ttest
# Paired t-test
# 
# data:  whole_estimates$Epithelial and est$Epi
# t = -5.8354, df = 17, p-value = 1.987e-05
...
## Fix things in encode-reinius and make the dataset long
#rename the columns to Immune/Epithelial
est$Immune <- est$IC
est$Epithelial <- est$Epi

#remove the old immune/epith columns
est <- est[, c("sample_id", "Immune", "Epithelial")]
#now there are 3 columns of sample ID, immune predictions, and epithelial predictions
...
ggplot(data = enc_rein_long, aes(x = est_celltype, y = estimates, group = sample_id))+
  geom_point(aes(color = as.factor(sample_id)), size = 3)+
  geom_line(aes(color = as.factor(sample_id)), size = 1)+
  xlab("")+
  ylab("Cell Type (%)")+
  scale_y_continuous(breaks = seq(0,120,10),
                     limits = c(0,120))+ #set the y axis from 0-100 labeled every 10 units
