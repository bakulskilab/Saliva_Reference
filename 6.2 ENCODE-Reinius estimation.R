# Purpose:  make a plot showing encode/reinius estimates for all 4 saliva sample types
#
...
#Fix IC, Epi to real names and flip them because I got them backwards
table(enc_rein_long$cell_type_est) #60 of each
enc_rein_long$cell_type_est <- gsub("Epi", "Immune", enc_rein_long$cell_type_est)
enc_rein_long$cell_type_est <- gsub("IC", "Epithelial", enc_rein_long$cell_type_est)
